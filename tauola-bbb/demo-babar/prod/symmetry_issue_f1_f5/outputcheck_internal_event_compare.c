...
//        for (int i=1; i<=18; i++) {out.write(& dane[0], dane.length()); out<<"\n"; getline(in, dane);}
 ...
 //       What I read is correct - checked
        ...
        TLorentzVector v1PI(px[7], py[7], pz[7], energy[7]);
        TLorentzVector v2PI(px[8], py[8], pz[8], energy[8]);
        TLorentzVector v3PI(px[9], py[9], pz[9], energy[9]);
        ...
        vWP.Boost(tauBoostP);
         ...
         if(print<10 && ij==printSwitch){
           cout<<"Tau Restframe"<<endl;
            cout<<"Momenta_tau="<<vTauN.Px()<<" "<<vTauN.Py()<<" "<<vTauN.Pz()<<endl;
            cout<<"Momenta_tau="<<vTauN.Px()<<" "<<vTauN.Py()<<" "<<vTauN.Pz()<<endl;
        ...
        if(print<10 && ij==printSwitch){
          cout<<"Hadronic Rest Frame"<<endl;
          cout<<"Momenta_W="<<vWP.Px()<<" "<<vWP.Py()<<" "<<vWP.Pz()<<" "<<vWP.E()<<endl;
          cout<<"Momenta_v1P="<<v1P.Px()<<" "<<v1P.Py()<<" "<<v1P.Pz()<<" "<<v1P.E()<<endl;
          cout<<"Momenta_v1PI="<<v1PI.Px()<<" "<<v1PI.Py()<<" "<<v1PI.Pz()<<" "<<v1PI.E()<<endl;
          cout<<"Momenta_v2P="<<v2P.Px()<<" "<<v2P.Py()<<" "<<v2P.Pz()<<" "<<v2P.E()<<endl;
          cout<<"Momenta_v2PI="<<v2PI.Px()<<" "<<v2PI.Py()<<" "<<v2PI.Pz()<<" "<<v2PI.E()<<endl;
          cout<<"Momenta_v3P="<<v3P.Px()<<" "<<v3P.Py()<<" "<<v3P.Pz()<<" "<<v3P.E()<<endl;
          cout<<"Momenta_v3PI="<<v3PI.Px()<<" "<<v3PI.Py()<<" "<<v3PI.Pz()<<" "<<v3PI.E()<<endl;
        ...
        double modPi1P = abs(pow(0.77590,2)-(pow(v1PI.Energy()+v3PI.Energy(),2) - pow(v1PI.Px()+v3PI.Px(),2) - pow(v1PI.Py()+v3PI.Py(),2) - pow(v1PI.Pz()+v3PI.Pz(),2)));
        double modPi2P = abs(pow(0.77590,2)-(pow(v2PI.Energy()+v3PI.Energy(),2) - pow(v2PI.Px()+v3PI.Px(),2) - pow(v2PI.Py()+v3PI.Py(),2) - pow(v2PI.Pz()+v3PI.Pz(),2)));
        double modPi3P = abs(pow(0.77590,2)-(pow(v1PI.Energy()+v2PI.Energy(),2) - pow(v1PI.Px()+v2PI.Px(),2) - pow(v1PI.Py()+v2PI.Py(),2) - pow(v1PI.Pz()+v2PI.Pz(),2)));

        TVector3 pi1PI(v1PI.Px(), v1PI.Py(), v1PI.Pz());
        ...
        if(ij<4) {
         ...
         if(modPi1P<modPi2P) {
            TVector3 n_perpendicularP;
            n_perpendicularP = pi1PI.Cross(pi2PI);
            double betaP = n_perpendicularP.Angle(vNeutInt);
            h1[ij+2]->Fill(cos(betaP));
            beta_Internal = betaP;
         } else {
            TVector3 n_perpendicularP;
            n_perpendicularP = pi2PI.Cross(pi1PI);
            double betaP = n_perpendicularP.Angle(vNeutInt);
            h1[ij+2]->Fill(cos(betaP));
            beta_Internal = betaP;
        }

        if(print<100 && ij==printSwitch){
          cout<<"Beta="<<beta_External-beta_Internal<<endl;
          print++;
        }
  ...
  TCanvas *beta_dist2 = new TCanvas("beta_distribution Interenal", "beta_distribution Internal", 800, 600);
  beta_dist2->cd();
  h1[2]->SetLineColor(kOrange);
  h1[2]->SetTitle("Beta distribution Internal");
